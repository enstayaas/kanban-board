...
import (
	...
	"kanban/internal/utils" // Подключаем твой логгер
...
func (h *LabelHandler) GetLabels(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		sendError(w, "Параметр board_id обязателен и должен быть числом", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		utils.LogError(err, "GetLabels: failed to fetch labels")
		sendError(w, "Ошибка при получении меток", http.StatusInternalServerError)
		return
	}
	defer rows.Close()
...
func (h *LabelHandler) CreateLabel(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		sendError(w, "Некорректный формат запроса", http.StatusBadRequest)
		return
	}

	if req.Name == "" {
		sendError(w, "Имя метки обязательно", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		utils.LogError(err, "CreateLabel: failed to insert label")
		sendError(w, "Не удалось создать метку", http.StatusInternalServerError)
		return
	}
...
func (h *LabelHandler) UpdateLabel(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.LogError(err, "UpdateLabel: failed to update label")
		sendError(w, "Не удалось обновить метку", http.StatusInternalServerError)
		return
	}
...
func (h *LabelHandler) DeleteLabel(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.LogError(err, "DeleteLabel: failed to delete label")
		sendError(w, "Не удалось удалить метку", http.StatusInternalServerError)
		return
	}

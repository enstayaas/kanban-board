...
// GET /tasks — Получить активные задачи по колонке (с пагинацией, поиском и метками)
func (h *TaskHandler) GetTasks(w http.ResponseWriter, r *http.Request) {
	...
	query := fmt.Sprintf(`
		SELECT id, column_id, title, description, priority, position, done_at, deadline, assigned_to
		FROM tasks 
		WHERE column_id=$1 
	...
	for rows.Next() {
		...
		err := rows.Scan(&t.ID, &t.ColumnID, &t.Title, &t.Description, &t.Priority, &t.Position, &t.DoneAt, &t.Deadline, &assignedToOpt)
		if err != nil {
...
func (h *TaskHandler) CreateTask(w http.ResponseWriter, r *http.Request) {
	...
	var maxPos int
	h.DB.QueryRow("SELECT COALESCE(MAX(position), 0) FROM tasks WHERE column_id=$1 AND deleted_at IS NULL", t.ColumnID).Scan(&maxPos)

	var err error
	var taskID int

	query := `
		INSERT INTO tasks (column_id, title, description, priority, position, deadline, assigned_to)
		VALUES ($1, $2, $3, $4, $5, $6, NULLIF($7, 0))
		RETURNING id`

	err = h.DB.QueryRow(query, t.ColumnID, t.Title, t.Description, t.Priority, maxPos+1, t.Deadline, t.AssignedTo).Scan(&taskID)
...
// PUT /tasks/{id} — Обновление и перемещение задачи с транзакцией
func (h *TaskHandler) UpdateTask(w http.ResponseWriter, r *http.Request) {
	...
	_, err = tx.Exec(query,
		...
		updateColumnID, finalPosition, t.Deadline, t.AssignedTo, taskID)
...
// DELETE /tasks/{id} — Мягкое удаление в корзину с транзакцией
...
// GET /tasks/archive — Получить список архивных (удалённых) задач
...
// DELETE /tasks/{id}/permanent — Полное удаление задачи из БД
...
func (h *TaskHandler) GetTaskLabels(w http.ResponseWriter, r *http.Request) {
	...
	// Теперь передаем число taskID — ошибка исчезнет!

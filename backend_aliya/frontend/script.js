...
//     // ===== ЗАГРУЖАЕМ МЕТКИ В МОДАЛЬНОЕ ОКНО =====
//     const container = document.getElementById('modalTaskLabels');
//     if (container && labels && labels.length > 0) {
//         // Получаем метки задачи (если функция getTaskLabels определена)
//         let taskLabelIds = [];
//         if (typeof getTaskLabels === 'function') {
//             const taskLabels = await getTaskLabels(task.id);
//             taskLabelIds = taskLabels.map(l => l.id);
...
//     const container = document.getElementById('modalTaskLabels');
//     if (container) {
//         // Проверяем, что метки уже загружены (из labels.js)
//         if (typeof labels !== 'undefined' && labels.length > 0) {
//             // Получаем метки, уже назначенные задаче
//             let taskLabelIds = [];
//             if (typeof getTaskLabels === 'function') {
//                 const taskLabels = await getTaskLabels(task.id);
//                 taskLabelIds = taskLabels.map(l => l.id);
...
//             container.innerHTML = labels.map(label => `
//                 <div class="modal-label-item">
//                     <label style="background: ${label.color}; padding: 4px 12px; border-radius: 20px; color: white;">
//                         <input type="checkbox" value="${label.id}" 
//                             ${taskLabelIds.includes(label.id) ? 'checked' : ''}
//                             onchange="toggleTaskLabel(${task.id}, ${label.id}, this.checked)">
//                         ${escapeHtml(label.name)}
